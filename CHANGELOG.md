...
-   **Trailing resume ask reconstruction** — rebuilt `ui_messages.json` now ends with the correct resume ask
    (`resume_task` for interrupted/active tasks, `resume_completed_task` for completed ones), mirroring
    Zoo-Code `Task#resumeTaskFromHistory`, so reopened tasks render correctly without the plugin re-adding it.
    `verify-ui-sync` now feeds the disk task status into the reconstruction to avoid false `ui_sync_mismatch`.

-   **`[ERROR]` tool-reminder omission** — `rebuildUiMessages` now drops the injected
    `[ERROR] You did not use a tool in your previous response!` reminder (plain, JSON-enveloped, and
    `<user_message>`-wrapped forms) instead of emitting it as a UI event.
-   **Interrupted-task timestamp** — the synthetic `Task was interrupted before completion.` `tool_result` now
    inherits a real epoch timestamp from the last ACH turn instead of the `ts: 0`-derived counter value.
-   **`<user_message>` wrapper stripping** — emitted `text` fields no longer carry the leading
    `<user_message>` / trailing `</user_message>` wrapper.
