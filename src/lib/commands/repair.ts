...
export async function action(taskId: string | undefined, cmdOpts: RepairCmdOptions): Promise<void> {
    ...
    if (modeCount !== 1) {
        ...
        console.error("  repair <taskId> [--force] [--no-backup] [--force-uim] [--fixed-input-token <n>] [--force-rebuild-hi]")
        console.error("  repair --all [--force] [--no-backup] [--verbose] [--fixed-input-token <n>] [--verify-ui-sync] [--force-rebuild-hi]")
        process.exit(1)
