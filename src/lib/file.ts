...
function snapshotMatch(a: FileSnapshot, b: FileSnapshot): string | null {
	...
	if (a.mtimeMs !== b.mtimeMs) s.push(`mtime: ${a.mtimeMs} <> ${b.mtimeMs}`)

	if (a.ctimeMs !== b.ctimeMs) s.push(`ctimeMs: ${a.ctimeMs} <> ${b.ctimeMs}`)

	if (a.size !== b.size) s.push(`size: ${a.size} <> ${b.size}`)

	if (a.inode !== b.inode) s.push(`inode: ${a.inode} <> ${b.inode}`)
...
export async function listBackups(tasksDir: string, opts: ListBackupsOptions = {}): Promise<BackupEntry[]> {
	const taskDirs = opts.taskId ? [path.join(tasksDir, opts.taskId)] : listTaskDirs(tasksDir)
 ...
 */
export async function listBackupsForTask(dir: string, basenames?: string[]): Promise<BackupScanEntry[]> {
