 ...
 */
export const historyItemForRepair = historyItemSchema
	.extend({
	...
	.superRefine((item, ctx) => {
		...
		if (
			...
			item.completedByChildId !== null &&
			!isUuid(item.completedByChildId)
		) {
			ctx.addIssue({
				code: ZodIssueCode.custom,
 ...
 */
export function validateHistoryItem(data: unknown, fullIndex?: EntryMap): ValidationResult {
	...
	const result = zodResultToValidationResult(parsed.success ? null : parsed)
