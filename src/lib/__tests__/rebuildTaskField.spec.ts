...
describe("extractTaskFromApiHistory", () => {
	...
	it("returns null when no user turn exists", () => {
		expect(extractTaskFromApiHistory([{ role: "assistant", content: [{ type: "text", text: "Hi" }] }])).toBeNull()
	})
	...
	it("skips non-object turns gracefully", () => {
		const result = extractTaskFromApiHistory([
			...
			{
				role: "user",
				content: [{ type: "text", text: "<user_message>Valid</user_message>" }],
			},
		])
	...
	it("returns null for turn without content array", () => {
		expect(extractTaskFromApiHistory([{ role: "user" }])).toBeNull()
	})
})
