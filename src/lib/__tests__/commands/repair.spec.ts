...
const mockIndexTransaction = vi.hoisted(() => vi.fn(function (this: any, _readOnly?: boolean) {
    ...
    describe("taskId mode", () => {
        it("dry-run: shows [DRY-RUN] would repair", async () => {
            mockRepairTaskDir.mockReturnValue({
                taskId: "t1", uiRepaired: true, taskRepaired: false,
                sizeRepaired: false, tokensRepaired: false, errors: [], backups: [],
            })
        ...
        it("force: shows repaired without [DRY-RUN]", async () => {
            mockRepairTaskDir.mockReturnValue({
                taskId: "t1", uiRepaired: true, taskRepaired: true,
                sizeRepaired: false, tokensRepaired: false, errors: [], backups: [],
            })
        ...
        it("errors path: shows errors and backups", async () => {
            mockRepairTaskDir.mockReturnValue({
                ...
                errors: ["missing ACH"], backups: ["/fake/root/tasks/t1/history_item.json.bak"],
            })
    ...
    describe("--all mode", () => {
        it("dry-run: shows [DRY-RUN] prefix and dry-run message", async () => {
            mockRepairAllCorrupted.mockReturnValue({
                total: 1, repaired: 1, failed: 0, unrepairable: 0,
                results: [{taskId: "t1", uiRepaired: true, errors: [], unrepairable: false}],
                indexEntries: 0, indexAdded: [], indexRemoved: [],
            })
