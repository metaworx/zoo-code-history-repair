...
const mockIdxRemoveById = vi.hoisted(() =>
	vi.fn((id: string) => {
...
const mockIdxConstructor = vi.hoisted(() =>
...
describe("delete command", () => {
	...
	it("force: strips array-format index entry with backup", async () => {
		mockIdxGetEntries.mockReturnValue([{ id: "task-1" }, { id: "task-2" }, { id: "task-3" }])
	...
	it("force: strips index entry with backup disabled", async () => {
		mockIdxGetEntries.mockReturnValue([{ id: "a" }, { id: "b" }])
