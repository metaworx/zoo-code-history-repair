/**
 * @file src/lib/__tests__/repairTask.spec.ts
 */
...
describe("repairTaskDir", () => {
	...
	it("returns error when api_conversation_history.json is missing", async () => {
		...
		expect(result.errors).toContain("missing or invalid api_conversation_history.json — cannot repair")
	...
	it("repairs placeholder task text from ACH", async () => {
		writeJson("api_conversation_history.json", [
			{
				role: "user",
				content: [{ type: "text", text: "<user_message>Fix the login bug</user_message>" }],
				ts: 100,
			},
		])
	...
	it("recomputes incorrect size", async () => {
		writeJson("api_conversation_history.json", [{ role: "user", content: [{ type: "text", text: "Hi" }], ts: 100 }])
	...
	it("handles all three repairs combined", async () => {
		writeJson("api_conversation_history.json", [
			{
				role: "user",
				content: [{ type: "text", text: "<user_message>Complete redesign</user_message>" }],
				ts: 100,
			},
		])
	...
	it("--force-rebuild-hi without an extractable task stays unrepairable", async () => {
		...
		expect(result.errors).toContain(
			"missing history_item.json and no task extractable from api_conversation_history.json — cannot rebuild",
		)
	})
})
