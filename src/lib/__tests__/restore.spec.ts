/**
 * @file src/lib/__tests__/restore.spec.ts
 */

import fs from "node:fs"
import os from "node:os"
import path from "node:path"
import { deleteBackups, listBackupsForType, restoreFromBackups } from "../restore.js"
...
describe("restoreFromBackups", () => {
	...
	it("creates a safety backup before overwriting", async () => {
		...
		const bakFiles = files.filter((f) => /^history_item\.json\.\d{8}-\d{6}\.bak\.json$/.test(f))
