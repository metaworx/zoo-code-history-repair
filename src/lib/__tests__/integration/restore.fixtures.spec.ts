...
import { copyFixtureTasks, createTempDir, read, touch, writeJson } from "../testHelpers.js"
...
describe("restore against fixtures (integration)", () => {
	...
	describe("listBackupsForType", () => {
		it("finds history_item backups by default", async () => {
			...
			touch(path.join(d, "history_item.json.20260808-054500.bak.json"), "backup content")
	...
	describe("restoreFromBackups", () => {
		...
		it("history_item restore creates a safety backup before overwriting", async () => {
			...
			const bakFiles = fs.readdirSync(d).filter((f) => /^history_item\.json\.\d{8}-\d{6}\.bak\.json$/.test(f))
		...
		it("_index.task restore creates a safety backup of the prior history_item.json", async () => {
			...
			const safetyFiles = fs.readdirSync(d).filter((f) => /^history_item\.json\.\d{8}-\d{6}\.bak\.json$/.test(f))
	...
	describe("deleteBackups", () => {
		...
		it("deletes only the requested type", async () => {
			...
			expect(fs.existsSync(path.join(d, "_index.task.20260808-054500.bak.json"))).toBe(false)
		})
