...
export default (tasksDir: string, consoleLogSpy: ReturnType<typeof vi.spyOn>) => async () => {
    ...
    expect(corruptions1.some(c => c.taskId === orphanId), "orphan must be in scan --short before delete").toBe(true);
    ...
    expect(corruptions3.length, "scan --short must remain with 1 folder-orphan residual after idempotent delete").toBe(1);
};
