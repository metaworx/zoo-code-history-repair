...
import {
    ...
    copyFixtureTasks,
    createTempDir,
    FIXTURE_LIST_AFTER_FILE,
    FIXTURE_LIST_BEFORE_FILE,
...
function getJsonOutput(spy: ReturnType<typeof vi.spyOn>): unknown {
...
describe("full pipeline (integration)", () => {
    ...
    it("scan→repair→validate cycle reduces corruption and is idempotent", () => {
        ...
        for (const id of Object.keys(CORRUPT_REASONS)) {
            ...
            expect(line, `${id} must appear in pre-repair list-corrupt`).toBeDefined();
        ...
        for (const id of HEALTHY) {
            ...
            expect(corruptLine, `${id}: healthy task must not appear in list-corrupt`).toBeUndefined();
        }
        ...
        expect(readJson(FIXTURE_LIST_BEFORE_FILE)).toDeepEqualJson(lcJson1, {
            ignoreProps: ['version'],
        }, "list-corrupt before")
        ...
        expect(readJson(FIXTURE_LIST_AFTER_FILE)).toDeepEqualJson(lcJson2, {
            ignoreProps: ['version'],
        }, "list-corrupt fixed")
        ...
        const r2RepairedIds = r2Lines
            .map(l => l.slice(0, 36))
            .filter(id => /^[0-9a-f-]{36}$/.test(id));

        // The detail task specifically must not be repaired again
        expect(
            r2RepairedIds.includes(detailTaskId),
            `${detailTaskId} must not be repaired again`,
        ).toBe(false);

        // Placeholder task must not be repaired again
        expect(
            r2RepairedIds.includes(placeholderId),
            `${placeholderId} must not be repaired again`,
        ).toBe(false);
        ...
        expect(readJson(FIXTURE_LIST_AFTER_FILE)).toDeepEqualJson(lcJson3, {
            ignoreProps: ['version'],
        }, "list-corrupt idempotency")
