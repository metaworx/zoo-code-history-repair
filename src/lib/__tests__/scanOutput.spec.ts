...
import { formatPerFieldSummary, perFieldRecoverability, type PerFieldRecoverability } from "../scanOutput.js"
...
describe("perFieldRecoverability", () => {
	...
	it("estimates token fields from ACH with medium confidence and extracts the task", async () => {
		...
		const c = corruption({ id: "task-1", tokensIn: 0, tokensOut: 0, totalCost: 0, apiConfigName: "deepseek" })
		const r = await perFieldRecoverability(c)
