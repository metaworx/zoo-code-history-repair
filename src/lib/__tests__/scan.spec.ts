...
describe("scanStorage", () => {
	...
	it("reports index_orphan when index entry has no folder", async () => {
		fs.writeFileSync(path.join(tasksDir, "_index.json"), JSON.stringify([{ id: "ghost", task: "Ghost", size: 0 }]))
	...
	it("detects placeholder task names in index", async () => {
		...
		fs.writeFileSync(path.join(dir, "history_item.json"), JSON.stringify(entry))
		fs.writeFileSync(path.join(tasksDir, "_index.json"), JSON.stringify([entry]))

		const result = await scanStorage(root)
		const hit = result.corruptions.find((c) => c.taskId === id)
		expect(hit?.reasons).toEqual(
			expect.arrayContaining([
				{ reason: "placeholder_task_name", source: "hi,idx" },
				{
					reason: "zero_size",
					source: "hi,idx",
				},
