/**
 * @file src/lib/__tests__/validate/historyItem.spec.ts
 */
...
describe("validateHistoryItem", () => {
	...
	it("status=delegated without delegatedToId → error", () => {
		...
		expect(r.valid).toBe(false)
		expect(r.issues.some((i) => i.code === "STATUS_DELEGATED_MISSING" && i.field.includes("delegatedToId"))).toBe(
			true,
		)
	})
	...
	it("cross-ref with fullIndex → dangling ref → error", () => {
		const fullIndex = new Map([["019f0f12-02f9-70df-a35e-2b110efe4107", makeValidEntry()]])
		const r = validateHistoryItem(
			makeValidEntry({
