 ...
 *
 * Extended with:
 *  - `stringify` option (skip JsonStreamStringify for pre-serialized strings)
 *  - Returns `{backupPath: string | null}` — caller controls backup retention
 */
...
export interface SafeWriteJsonOptions {
	...
	/**
	 * When false, data is already a string — write directly without JsonStreamStringify.
	 * When true or undefined, stream-serialize via json-stream-stringify.
	 * @default true
	 */
	stringify?: boolean

	/**
	 * When true, the backup file is NOT deleted on success — its path is returned
	 * in the result so the caller can rename or delete it.
	 * When false or undefined (default), the backup is deleted as in the original implementation.
	 * @default false
	 */
	keepBackup?: boolean
}

export interface SafeWriteJsonResult {
	/**
	 * Path to the `.bak_*.tmp` file if a backup was created on overwrite.
	 * Null if the target file did not exist (first write).
	 * The caller is responsible for renaming or deleting this file.
	 */
 ...
 * - When `stringify` is false, data is written directly as a string (no JSON serialization).
 * - Returns the backup path so the caller can rename or delete it.
 *
 * @param {string} filePath - The absolute path to the target file.
 * @param {any} data - The data to serialize to JSON and write, or a pre-serialized string if `stringify` is false.
...
async function safeWriteJson(
	...
	try {
		...
		await _streamDataToFile(actualTempNewFilePath, data, options?.prettyPrint, options?.stringify ?? true)
		...
		return { backupPath: actualTempBackupFilePath }
	} catch (originalError) {
...
/**
 * Helper function to stream data to a file.
 * @param targetPath The path to write the stream to.
 * @param data The data to stream (object for JSON serialization, or string if stringify=false).
 * @param prettyPrint Whether to format the JSON with indentation.
 * @param stringify When false, data is already a string — write directly.
 * @returns Promise<void>
 */
async function _streamDataToFile(targetPath: string, data: any, prettyPrint = false, stringify = true): Promise<void> {
	// Stream data to avoid high memory usage for large JSON objects.
	const fileWriteStream = fsSync.createWriteStream(targetPath, { encoding: "utf8" })

	if (!stringify) {
		// Data is already a string — write directly, no JSON serialization needed
		return new Promise<void>((resolve, reject) => {
			fileWriteStream.on("error", reject)
			fileWriteStream.on("finish", resolve)
			fileWriteStream.end(String(data))
		})
	}
