...
import { backupTimestamp, consolidateBackups, JsonFileTransaction, mapTypeToFileName, readJsonFile } from "./file.js"
...
export class IndexTransaction extends JsonFileTransaction {
	...
	async save(validate: boolean = true, backup: boolean = true): Promise<string | null> {
		...
		if (this.data && typeof this.data === "object") {
			...
			if (d.entries && Array.isArray(d.entries)) {
				;(d.entries as Array<Record<string, unknown>>).sort(
					(a, b) => ((b.ts as number) ?? 0) - ((a.ts as number) ?? 0),
				)
			}
		}
	...
	async replaceId(
		...
		if (validate) {
			...
			if (result.errorCount > 0) {
				throw new Error(
					`validation failed for index entry ${id}: ${result.issues
	...
	async repair(
		...
		const uiSyncMismatches = options.verifyUiSync ? await this._verifyUiSync(newIndex) : []
