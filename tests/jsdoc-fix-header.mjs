...
const DEFAULT_IGNORES = ["**/node_modules/**", "dist/**", "tests/fixtures/**"]

console.error(`\n${yargs(hideBin(process.argv)).help(false).argv.$0} v${VERSION}\n`)
...
const argv = yargs(hideBin(process.argv))
	...
	.option("fix-all", {
		...
		default: false,
		describe:
			"Alias for --fix-position --fix-missing: Move existing header and ADD default header (--test is irrelevant here)",
	})
	.option("fix-test", {
		alias: "T",
		type: "string",
	...
	.example(
		"$0 --fix",
		`Fix header position showing file path of changed and erroneous files, add missing header to ${JSON.stringify(DEFAULT_TEST_GLOB)}`,
	)
...
for (const file of files) {
	...
	if (isFixMode && !quiet) {
		...
		console.log(
			`fix: ${action === "add" ? "m" : action === "move" ? "p" : "-"} ${hasShebang ? "s" : "-"} ${relative}`,
		)
...
function normalizePattern(argument, result = []) {
	...
	argument.forEach((p) => {
		...
		if (fs.existsSync(p)) {
			result.push(p.replace(/\\/g, "/").replace(/^\.?\/?/, ""))
