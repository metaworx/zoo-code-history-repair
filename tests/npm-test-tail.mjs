  ...
  .command(
    ...
    ( y ) =>
    {
      y
        ...
        .positional( 'extra', {           // ← collects everything after command
          type:     'string',
          array:    true,
...
const RELEVANT_KEYWORDS_REGEX = new RegExp(
...
async function main()
{
  ...
  npm.on( 'close', ( code ) =>
  {
    ...
    let linesToShow = wantLast
      ? buffer.slice( Math.max( 0, buffer.length - maxLines ) )
      : buffer.slice( 0, maxLines );
